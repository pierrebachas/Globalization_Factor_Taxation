...
*** 5.1 Main analysis outputs


			*** 5.1.1 Joint ES graphs:
			{
			cd "$usedata/synthetic_ES_intermediat_data/main_outcomes"
			use trade, clear
			{
			local v trade
			{
			...
			***
			...
			twoway 	rconnected ci4 ci4 rel_year if rel_year==-10, msymbol(i) color(dkgreen)  || 
					...
					rconnected ci4 ci4 rel_year if rel_year==10, msymbol(i) color(dkgreen)  || 
					scatter est_es_`v' rel_year, lcolor(black) mcolor(dkgreen)  || , 
					text(.1 -5 "Joint significance of event dummies:" " `fstat'" "(`pval')", justification(left) size(2.8)) 
					`xtitle' 
					`ytitle'	
					`xaxis'
					`yaxis'
			...
			***
			...
			// Manually reduce ci4 to within limits for visibility of figure
			replace ci4 = -0.08 if rel_year==-10 & _n ==1
			replace ci4 = 0.12 if rel_year==-10 & _n ==22
			...
			***
			...
			local xtitle "xtitle("Years to/from Event",  margin(med) size(medlarge))"
			local ytitle "ytitle("Coefficient on Value Added Corp.",  margin(med) size(medlarge))"
			local yaxis "ylabel(-0.08(0.04)0.12, nogrid gmin gmax) yscale(range(-0.08 0.12)) yline(-0.08(0.04)0.12, lstyle(minor_grid) lcolor(gs1)) yline(0) "
			local xaxis "xline(-0.5) xlabel(-10(2)10)"	
			...
			local xtitle "xtitle("Years to/from Event", margin(medsmall))"
			local ytitle "ytitle("Value Added, Corporate Sector", margin(medsmall))"
			local yaxis "ylabel(.40(.04).60, nogrid gmin gmax) yscale(range(.40 .60)) yline(.40(.04).60, lstyle(minor_grid) lcolor(gs1))"
			local xaxis "xline(-0.5) xlabel(-10(2)10)"	
