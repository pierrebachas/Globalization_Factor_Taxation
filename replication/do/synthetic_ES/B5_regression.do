...
*** 4. Regression 
	...
	local var="$var"
		...
		* 4.3 Imputation method
		...
		/*
		cd "$usedata"
		use cross_country_final, clear

		replace vat_intro_year=. if cty_id>100

		did_imputation tax_trade cty_id year vat_intro_year, allhorizon autosample

		This way we get a warning message that less than 30 treated units per FE may result in 
		downward biased SE's. For some reason, this warning is not issued when we run our regression 
		without the minn(0) option. 
		*/
		...
		****
		...
		cd "$usedata"
		save "synthetic_ES_intermediat_data/cap_lib_events/`var'.dta", replace
