...
*** 5.2 Mechanism outputs, capital liberalization events



		* 5.1 Joint ES graphs
		{
		cd "$usedata/synthetic_ES_intermediat_data/cap_lib_events_additional_outcomes"
		...
		***
		...
		cd "$outputs/capital_lib_events/es_graphs_additional_outcomes"
		...
		local yaxis "ylabel(-.1(.05).1, nogrid gmin gmax) yscale(range(-0.1 0.1)) yline(-.1(.05).1, lstyle(minor_grid) lcolor(gs1))"
		local xaxis "xline(-0.5) xlabel(-10(2)10)"	
		...
		***
		...
		local xtitle "xtitle("Years to/from Event", margin(med) size(medlarge))"
		local ytitle "ytitle("Coefficient on Corporate profits", margin(med) size(medlarge))"
		local yaxis "ylabel(-0.1(0.05)0.1, nogrid gmin gmax) yscale(range(-0.1 0.1)) yline(-0.1(0.05)0.1, lstyle(minor_grid) lcolor(gs1)) yline(0) "
		local xaxis "xline(-0.5) xlabel(-10(2)10)"	
		...
		local xtitle "xtitle("Years to/from Event", margin(med) size(medlarge))"
		local ytitle "ytitle("Corporate profits", margin(med) size(medlarge))"
		...
		***
		...
		local yaxis "ylabel(.15(.05).25, nogrid gmin gmax) yscale(range(.15 .25)) yline(.15(.05).25, lstyle(minor_grid) lcolor(gs1))"
		local xaxis "xline(-0.5) xlabel(-10(2)10)"	
		...
		* 5.2 Table
		...
		cd "$outputs/capital_lib_events/tables"

		esttab pt_cit_rate_winz pt_selfemployed pt_os_corp pt_os_hh pt_ce_hh pt_mi_hh  pt_industry_va pt_services_va pt_agric_va ///
				using synth_post_treat_cap_events_additional_outcomes.tex ///
				, se replace keep(post_treat) nonotes ///
				nocons nonumber nomtitles star(* 0.1 ** 0.05 *** 0.01)


		esttab imp_cit_rate_winz imp_selfemployed imp_os_corp imp_os_hh imp_ce_hh imp_mi_hh imp_industry_va imp_services_va imp_agric_va ///
				using synth_post_treat_cap_events_additional_outcomes.tex ///
				, se append keep(tau) ///
				nocons nonumber  nomtitles star(* 0.1 ** 0.05 *** 0.01)		
